...
class ppcThree {
  ...
  async cssHtmlInit(){
    ...
    document
    .getElementById(this.threeConfig.id).appendChild(this.cssRenderer.domElement);
  ...
  createCube() {
    ...
    const material = this.track(new THREE.MeshBasicMaterial({ color: 0x00ff00 }));
  ...
  render(camera) {
    ...
    this.controls = this.track(new OrbitControls(camera, this.renderer.domElement)); //创建控件对象
  ...
  useEffectComposer() {
    const outlinePass = this.track(new OutlinePass(
